...
@Service
public class BettingSearchCDCConsumer extends GenericService<BettingSearchModel,Long, BettingProduct> {

    private static final Logger logger = LoggerFactory.getLogger(BettingSearchCDCConsumer.class);
    private final ObjectMapper objectMapper;

    public BettingSearchCDCConsumer(BettingSearchRepository bettingSearchRepository, ObjectMapper objectMapper) {
        super(bettingSearchRepository, BettingProduct.class);
        this.objectMapper = objectMapper;
    }

    @KafkaListener(topics = "dbserver1.forecasthub.betting_product", groupId = "betting-product-search-group")
    @Transactional
    ...
    public void handleCreateOrUpdate(JsonNode jsonNode) throws IllegalAccessException {
        //TODO: 여기에 ID 바꾸면됨
        Long userId = jsonNode.get(IDs.BETTING_ID).asLong();
        saveOrUpdate(jsonNode, userId, BettingSearchModel.class);
    }
